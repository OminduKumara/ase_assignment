...
public class UserRepository : IUserRepository
{
    ...
    public async Task<User?> GetUserByLoginIdentifierAsync(string loginIdentifier)
    {
        ...
        var normalizedLogin = loginIdentifier.Trim();

        if (normalizedLogin.Contains("@", StringComparison.Ordinal))
        {
            return await GetUserByEmailAsync(normalizedLogin);
        }

        var user = await GetUserByUsernameAsync(normalizedLogin);
        if (user != null)
        {
            return user;
        }

        return await GetUserByIdentityNumberAsync(normalizedLogin);
    }
