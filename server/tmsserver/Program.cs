        ...
        using (var connection = new SqlConnection(connectionString))
        {
            ...
            using (var command = connection.CreateCommand())
            {
                command.CommandText = @"
                    ...
                    IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_users_identitynumber' AND object_id = OBJECT_ID('dbo.Users'))
                    BEGIN
                        CREATE INDEX idx_users_identitynumber ON dbo.Users(IdentityNumber);
                    END;
                ";
